...
curandGenerator_t gen, int grid_size, double *T, double K_FENE, double K_bend)
{
    ...
    printf("\n ***The Active Polymer MPCD_MD simulation(periodic + noSlip) is done*** By: Mahtab Taghavinejad, mahtabnejadt@gmail.com.");
    ...
    calc_accelaration(d_mdX , d_mdY, d_mdZ , d_Fx_holder , d_Fy_holder , d_Fz_holder ,d_Fx_bending , d_Fy_bending , d_Fz_bending, d_mdAx , d_mdAy , d_mdAz ,d_L , Nmd , n_md ,topology ,  ux ,h_md, grid_size, K_FENE, K_bend);
    cudaFree(d_tmp);
...
curandGenerator_t gen, int grid_size, double real_time, double *gama_T, int *d_random_array, unsigned int seed, int *flag_array, double u_scale, double K_FENE, double K_bend)
{
    ...
    if(BC == 1){
        Active_calc_acceleration(d_mdX , d_mdY, d_mdZ , d_Fx_holder , d_Fy_holder , d_Fz_holder , d_Fx_bending , d_Fy_bending , d_Fz_bending, d_mdAx , d_mdAy , d_mdAz ,d_fa_kx, d_fa_ky, d_fa_kz, d_fb_kx, d_fb_ky, d_fb_kz, d_Aa_kx, d_Aa_ky, d_Aa_kz, d_Ab_kx, d_Ab_ky, d_Ab_kz, d_ex, d_ey, d_ez, ux, density, gama_T, d_L , Nmd , m_md ,topology , real_time, grid_size,1 ,
