...
int main(int argc, const char* argv[])
{
    ...
    *scalefactor = 1.0;
    ...
    int *dn_mpcd_tot, *dn_md_tot;
    cudaMalloc((void**)&dn_mpcd_tot, sizeof(int));
    cudaMalloc((void**)&dn_md_tot, sizeof(int));
    double *N_avg;
    ...
    double *b_x, *b_y, *b_z;
    cudaMalloc((void**)&b_x, sizeof(double) * Nc); cudaMalloc((void**)&b_y, sizeof(double) * Nc); cudaMalloc((void**)&b_z, sizeof(double) * Nc);
    ...
    //this part is specific for when we don't have activity and no-slip boundary condition.
        ...
        for(int t = TIME/swapsize ; t<T; t++)
        {
            for (int i =0;i<int(swapsize/h_mpcd); i++)
            {
                ...
                /*noslip_MPCD_MD_collision2(d_vx , d_vy , d_vz , d_index,
                    d_mdVx , d_mdVy , d_mdVz , d_mdIndex,
                    d_ux , d_uy , d_uz , d_e , d_scalefactor , d_n , d_m ,
    ...
    //this part is specific for when we have activity and periodic boundary condition. 
        ...
        for(int t = TIME/swapsize ; t<T; t++) //T is TIME/swapsize + simulationtime/swapsize. it goes 10 steps from TIME/swapsize till end (in case swapsize is 100 and siumlationtime is 1000). 
        {
            ...
            for (int i =0;i<int(swapsize/h_mpcd); i++) //swapsize/h_mpcd when swapsize is 100 and h_mpcd is 0.1, is equal to 1000.
            {
                ...
                int ErrorFlag_mpcd_opp_opp_opp_opp = 0;
                ...
                d_n_outbox_mpcd, d_n_outbox_md, h_dn_mpcd_tot, h_dn_md_tot, CMsumblock_n_outbox_mpcd, CMsumblock_n_outbox_md, &ErrorFlag_mpcd, &ErrorFlag_mpcd_opp, &ErrorFlag_mpcd_opp_opp, &ErrorFlag_mpcd_opp_opp_opp, &ErrorFlag_mpcd_opp_opp_opp_opp, n_out_flag_mpcd, n_out_flag_mpcd_opp, &d_zero_mpcd);
                if (ErrorFlag_mpcd != 0){
                    // Handle error
        ...
        cudaFree(dn_mpcd_tot); cudaFree(n_mpcd_avg); cudaFree(sumblock_n_mpcd);
        cudaFree(dn_md_tot); cudaFree(n_md_avg); cudaFree(sumblock_n_md);
